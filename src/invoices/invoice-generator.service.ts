...
export class InvoiceGeneratorService {
  ...
  /**
   * Determines if the first invoice should be generated at activation time.
   * Only generates if the generation day (payday - 5) for the current period
   * has already arrived or passed. Otherwise the daily cron will generate it on time.
   */
  private shouldGenerateOnActivation(contract: Contract, currentDate: Date): boolean {
    const DAYS_BEFORE = 5;

    if (contract.mode === PaymentMode.MONTHLY) {
      const payday = contract.payday || 1;

      // Find the next payday date (this month if not passed, otherwise next month)
      let nextPayday = new Date(currentDate.getFullYear(), currentDate.getMonth(), payday);
      nextPayday.setHours(0, 0, 0, 0);
      const todayNorm = new Date(currentDate);
      todayNorm.setHours(0, 0, 0, 0);
      if (nextPayday < todayNorm) {
        nextPayday = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, payday);
        nextPayday.setHours(0, 0, 0, 0);
      }

      // Generation day = 5 days before the next payday
      const generationDate = new Date(nextPayday);
      generationDate.setDate(generationDate.getDate() - DAYS_BEFORE);

      // Only generate if we've reached or passed the generation date
      return todayNorm >= generationDate;
    }

    // For SEMIANNUAL/ANNUAL, defer to the exact-day cron logic
    return this.isGenerationDay(contract, currentDate);
  }

  /**
   * Generates the first invoice at activation, but only if it's time (payday - 5).
   * Returns null if it's not yet time (the daily cron will handle it later).
   */
  async generateFirstInvoiceIfDue(contractId: string): Promise<Invoice | null> {
    ...
    if (contract.package && contract.package.isBillable === false) return null;

    const currentDate = new Date();
    if (!this.shouldGenerateOnActivation(contract, currentDate)) {
      this.logger.log(`Contract ${contractId} activated but not yet generation day (payday-5). First invoice will be generated by the daily cron.`);
      return null;
    }

    // It's time — generate (generateInvoiceForContract handles duplicate check)
    try {
      return await this.generateInvoiceForContract(contractId);
    } catch (err) {
      // Duplicate or other guard — non-fatal
      this.logger.warn(`Could not generate first invoice for ${contractId}: ${err.message}`);
      return null;
    }
  }
